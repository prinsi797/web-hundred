...
<table class="table table-hover">
  ...
  <tbody>
    ...
    <tr class="row_{{ $v->id }}">
      ...
      <td>{{ ++$record_id }}</td>
      <td>{{$v->name}}<br/> {{ "@".$v->username}}</td>
      <td>{{$v->dob}}</td>
      <td>
        @if (strlen($v->phone_number) === 10)
        {{ '(' . substr($v->phone_number, 0, 3) . ') ' . substr($v->phone_number, 3, 3) . '-' . substr($v->phone_number, 6) }}
        @else
            {{ $v->phone_number }}
        @endif
    </td>
      ...
      <td>{{$v->total_exercise_count}}</td>
      <td>{{ Date('d M, Y',strtotime($v->created_at)) }}</td>
      {{-- <td>{{ Date('m/d/Y',strtotime($v->created_at)) }}</td> --}}
      <td>
        @if($v->deleted_at==null)
        <a href="{{$edit_route.'?id='.$v->id}}" class="btn btn-primary btn-sm">Edit</a>
        <a href="#" data-id="{{ $v->id }}" class="btn btn-danger btn-sm trash_btn delete{{ $v->id }}">Delete</a>
        <a href="#" data-id="{{ $v->id }}" class="btn btn-secondary btn-sm contacts_btn delete{{ $v->id }}">Contacts</a>
        <a href="#" data-id="{{ $v->id }}" class="btn btn-secondary btn-sm school_btn delete{{ $v->id }}">School</a>
        <a href="#" data-id="{{ $v->id }}" class="btn btn-secondary btn-sm friends_btn delete{{ $v->id }}">Friends</a>
        <a href="#" data-id="{{ $v->id }}" class="btn btn-secondary btn-sm status_btn delete{{ $v->id }}">Stats</a>
        @else
